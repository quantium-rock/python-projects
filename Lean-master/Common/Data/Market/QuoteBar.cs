...
namespace QuantConnect.Data.Market
{
    ...
    public class QuoteBar : BaseData, IBaseDataBar
    {
        ...
        private QuoteBar ParseQuote(SubscriptionDataConfig config, DateTime date, string line, bool useScaleFactor)
        {
            ...
            {
                // hourly and daily have different time format, and can use slow, robust c# parser.
                quoteBar.Time = DateTime.ParseExact(csv[0], DateFormat.TwelveCharacter, 
                    CultureInfo.InvariantCulture).ConvertTo(config.DataTimeZone, config.ExchangeTimeZone);
                //config.DataTimeZone --> UTC for Forex and CFD
            }
            else
            {
